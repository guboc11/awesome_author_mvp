import '../App.css';

export default function MainPageWithBorder() {
  const logoImagePath = "data:image/png;base64,"
  return (
    <div>

      <nav id="logo-navigation" className="border-4 border-red-500 flex" >
        <div className="border-4 border-yellow-500 w-1/2 flex items-center justify-center">
          <img src={logoImagePath}/>
        </div>
        <div className="border-4 border-yellow-500 w-1/2">
          <div className="flex items-center h-full">
            <div className="w-full flex justify-around">
              <button className="border-2 border-blue-500 text-3xl">Home</button>
              <button className="border-2 border-blue-500 text-3xl">My Page</button>
              <button className="border-2 border-blue-500 text-3xl">Writer</button>
              <button className="border-2 border-blue-500 text-3xl">Employer</button>
            </div>
          </div>
        </div>
      </nav>

      <section id="vision" className="border-4 border-red-500 bg-gray-200">
        <div className="border-4 border-yellow-500 h-32 flex items-center justify-center">
          <p className="text-5xl">Grow Your Vision</p>
        </div>
        <div className="border-4 border-yellow-500 py-10 flex flex-col justify-center">
          <p className="text-xl text-center">작가들을 위한 새로운 창작의 공간, 한 눈에 보는 작품 포트폴리오</p>
          <p className="text-xl text-center">장르별로 원하는 스타일의 작가 및 고용주와 연결해드립니다. 작가로서 나의 개성을 </p>
          <p className="text-xl text-center">기록으로 남겨보세요!</p>
        </div>
        <div className="border-4 border-yellow-500 h-48 flex items-center justify-around">
          <div className="border-2 bg-indigo-600 h-36 w-56 flex items-center justify-center">
            <p className="text-2xl text-white">작가로 시작하기</p>
          </div>
          <div className="border-2 bg-indigo-600 h-36 w-56 flex items-center justify-center">
            <p className="text-2xl text-white">의뢰인으로 시작하기</p>
          </div>
        </div>
      </section>

      <section id="inform" className="border-4 border-red-500">
        <div className='h-60'>
          <p className="text-5xl ml-20 mt-10">Inform</p>
          <p className="text-xl ml-20 mt-10">여기에 내용을 입력해 주세요.</p>
        </div>
      </section>

      <hr className="border-4 my-3 mx-3 rounded border-violet-500"></hr>

      <section id="team" className="border-4 border-red-500">
        <div className='h-60'>
          <p className="text-5xl ml-20 mt-10">Team</p>
          <p className="text-xl ml-20 mt-10">여기에 내용을 입력해 주세요.</p>
        </div>
      </section>

      <section id="contact" className="border-4 border-red-500 h-80 bg-black text-white flex justify-center items-center">
        <div className="border-4 border-yellow-500 w-1/2 mx-10">
          <div className="py-5">
            <p>Email</p>
            <p className="text-3xl">penment@gmail.com</p>
          </div>
          <div className="py-5">
            <p>Tel</p>
            <p className="text-3xl">010-0000-0000</p>
          </div>
        </div>
        <div className="border-4 border-yellow-500 w-1/2 mx-10">
          <div className='h-32 flex flex-col justify-around'>
            <p>Follow Us</p>
            <p className="text-3xl">Instagram</p>
          </div>
        </div>
      </section>

    </div>
  )
}
